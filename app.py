...
input_data = pd.DataFrame([{
    "battery_power": battery_power, "ram": ram, "internal_memory": internal_memory,
    "mobile_wt": mobile_wt, "px_height": px_height, "px_width": px_width,
    "sc_h": sc_h, "sc_w": sc_w, "talk_time": talk_time, "fc": fc, "pc": pc,
    "n_cores": n_cores, "clock_speed": clock_speed, "blue": int(blue),
    "dual_sim": int(dual_sim), "four_g": int(four_g), "three_g": int(three_g),
    "touch_screen": int(touch_screen), "wifi": int(wifi),
    "pixel_density": pixel_density, "screen_area": screen_area
}])
...
with col_pred:
    ...
    fig = go.Figure(go.Bar(
        x=proba, y=[PRICE_LABELS[i] for i in range(4)], orientation="h",
        marker_color=colors, text=[f"{p:.1%}" for p in proba], textposition="outside"
    ))
...
with col_chart:
    ...
    radar_values = [
        min(battery_power/5000,1)*100, min(ram/12288,1)*100,
        min(internal_memory/512,1)*100, min(pc/64,1)*100,
        min(n_cores/8,1)*100, min(clock_speed/3.0,1)*100,
        (100-min(mobile_wt/250,1)*100), min(fc/20,1)*100,
    ]
    cats = ["Battery","RAM","Storage","Camera","Cores","Speed","Light","Front Cam"]
    rv   = radar_values + [radar_values[0]]
    rc   = cats + [cats[0]]
    fig2 = go.Figure(go.Scatterpolar(
        r=rv, theta=rc, fill="toself", fillcolor="rgba(233,69,96,0.2)",
        line=dict(color="#e94560", width=2), marker=dict(size=6, color="#e94560")
    ))
    fig2.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0,100])),
        showlegend=False, height=380,
        margin=dict(l=30,r=30,t=30,b=30), paper_bgcolor="white"
    )
    st.plotly_chart(fig2, use_container
